...
def crop_data_top_down(images, pose2, pose3):
    ...
    for ii in xrange(num_data_points):
        ...
        horizSkw = np.random.uniform(0.55, 0.5)
        incSiz = np.random.uniform(180, 180)
        # hW /= 2
        hW += incSiz
        skw = [verSkw, horizSkw]
    ...
    for ii in xrange(num_of_data):
        ...
        for jj in xrange(14):
            for kk in xrange(FLAG.volume_res):
                vec_x[ii, jj, kk] = gaussian(kk, p3_[jj, 0], FLAG.sigma,
                                             FLAG.joint_prob_max)
                vec_y[ii, jj, kk] = gaussian(kk, p3_[jj, 1], FLAG.sigma,
                                             FLAG.joint_prob_max)
                vec_z[ii, jj, kk] = gaussian(kk, p3_[jj, 2], FLAG.sigma,
                                             FLAG.joint_prob_max)
